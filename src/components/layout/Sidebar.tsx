...
export const Sidebar = () => {
  ...
  return (
    <aside className="w-52 shrink-0 bg-white dark:bg-[#0b1120] border-r border-gray-200 dark:border-gray-800/60 flex flex-col py-5">
      <div className="px-5 mb-4">
        <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-gray-400 dark:text-gray-500">Menu</p>
      </div>
      <nav className="flex-1 px-3 space-y-0.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-indigo-600 dark:bg-indigo-400 rounded-full" />
              )}
      ...
      <div className="px-5 mt-4">
        <p className="text-[10px] text-gray-300 dark:text-gray-600">v1.0</p>
      </div>
    </aside>
  );
};
