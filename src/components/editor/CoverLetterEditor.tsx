    ...
    <div className="flex h-full">
      <div className="w-96 bg-white border-r border-gray-200 flex flex-col overflow-hidden shrink-0">
        <div className="px-4 py-2.5 border-b border-gray-200">
          <h2 className="font-semibold text-xs text-gray-900">Edit Cover Letter</h2>
        </div>
      ...
      <CollapsibleSection title="Body Paragraphs" icon={FileText} defaultOpen>
        <div className="space-y-2.5">
          {cl.paragraphs.map((para) => (
            <div key={para.id} className="p-3 rounded-md border border-gray-200 bg-gray-50/50 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-medium text-indigo-600">Paragraph {para.order + 1}</span>
          ...
          <button
            ...
            className="w-full py-2 text-xs text-gray-500 border border-dashed border-gray-300 rounded-md hover:border-indigo-400 hover:text-indigo-600 transition-colors flex items-center justify-center gap-1.5"
          >
...
const CoverLetterPreview = () => {
  ...
  const shared = { data: coverLetterData, pc: primaryColor, sc: secondaryColor, ac: accentColor, hf: headingFont, bf: bodyFont, text, mutedText, isDark: false };
