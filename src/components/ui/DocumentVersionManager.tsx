      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1.5 w-80 z-50 bg-white dark:bg-[#1a2332] border border-gray-200 dark:border-gray-700/60 rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-4 space-y-3 border-b border-gray-100 dark:border-gray-700/30">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Name..."
                  className="flex-1 px-2.5 py-1.5 text-xs bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-md text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
              ...
              <label className="flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-gray-600 rounded-md hover:border-indigo-400 hover:text-indigo-600 cursor-pointer transition-colors">
            ...
            <div className="max-h-60 overflow-y-auto">
              {versions.length === 0 ? (
                <div className="p-4 text-center text-xs text-gray-400 dark:text-gray-500">No saved versions</div>
              ) : (
                <div className="divide-y divide-gray-100 dark:divide-gray-700/30">
                  {versions.map((version) => (
                    <div key={version.id} className="p-3 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 min-w-0">
                          <FileText className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                          <div className="min-w-0">
                            <p className="text-xs font-medium text-gray-900 dark:text-gray-100 truncate">{version.name}</p>
                        ...
                        <div className="flex items-center gap-0.5 shrink-0">
                          <button onClick={() => loadVersion(version)} className="px-2 py-1 text-[10px] font-medium text-indigo-600 dark:text-indigo-400 rounded hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors">Load</button>
                          <button onClick={() => exportVersion(version)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"><Download className="w-3 h-3" /></button>
